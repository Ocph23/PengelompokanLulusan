...
namespace Kmeans
{
    ...
    public class IrisData
    {
        ...
        [LoadColumn(1)]
        public float Jurusan;

        [LoadColumn(2)]
        public float IPK;

        [LoadColumn(3)]
        public float Gelombang;

        [LoadColumn(4)]
        public float HasilTes;

        [LoadColumn(5)]
        public float MasaStudi;
        [LoadColumn(6)]
        public string Label;
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IDataView dataView = context.Data.LoadFromTextFile<IrisData>("./kmeanData.data", hasHeader: true, separatorChar: ',');
            string featuresColumnName = "Features";
            var pipeline = context.Transforms
                .Concatenate(featuresColumnName, "Suku", "Jurusan", "IPK", "Gelombang", "HasilTes", "MasaStudi")
                .Append(context.Clustering.Trainers.KMeans(featuresColumnName, numberOfClusters: 4));


            var model = pipeline.Fit(dataView);

            using (var fileStream = new FileStream("./IrisClusteringModel.zip", FileMode.Create, FileAccess.Write, FileShare.Write))
            {
            ...
            var prediction = predictor.Predict(new IrisData {
                  Suku=2,Jurusan=2,IPK=3.0f, Gelombang=1,HasilTes =5.0f,MasaStudi= 2.5f
            });
            ...
            var predictions = context.Data.CreateEnumerable<IrisData>(
            ...
            foreach (var p in predictions.Take(90))
                Console.WriteLine(
                    $"Suku: {p.Suku}, Jurusan: {p.Jurusan}, IPK: {p.IPK}, Gelombang: {p.Gelombang}, Hasil: {p.HasilTes}, Masa: {p.MasaStudi}");
