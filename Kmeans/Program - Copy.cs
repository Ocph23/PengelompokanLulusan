...
namespace Kmeans
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var m = Clustering.KMeans.Cluster(rawData,rawData.Length*10,4);
...
*//*
            ...
            int numClusters = 4;
            Console.WriteLine("\nJumlah Cluster : " + numClusters);

            int[] clustering = Cluster(rawData, numClusters); // this is it

            Console.WriteLine("\nK-means clustering complete\n");

            Console.WriteLine("Final clustering :\n");
...
*//*
        ...
        private static Tuple<bool,double[][]> UpdateClustering(double[][] data, int[] clustering, double[][] means)
        {
            ...
            if (changed == false)
                return Tuple.Create(false, newdata); // no change so bail and don't update clustering[][]
            ...
            for (int k = 0; k < numClusters; ++k)
                if (clusterCounts[k] == 0)
                    return Tuple.Create(false, newdata); // bad clustering. no change to clustering[][]

            Array.Copy(newClustering, clustering, newClustering.Length); // update
            return Tuple.Create(true, newdata); // good clustering and at least one change
        }
